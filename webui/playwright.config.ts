...
export default defineConfig({
    ...
    webServer: {
        ...
        command: 'bash -c "set -e; cd .. && rm -rf /tmp/zp-pw && mkdir -p /tmp/zp-pw && export ZEROPOINT_ROOT_PATH=/tmp/zp-pw ZEROPOINT_MODE=mock ZEROPOINT_AGENT_LOCAL=1; zeropoint-agent node ensure namespace settings -c name=settings --perms rw* >/dev/null; zeropoint-agent node ensure namespace modules -c name=modules --perms rw* >/dev/null; unset ZEROPOINT_AGENT_LOCAL; zeropoint-agent serve"',
