...
export default defineConfig({
    ...
    webServer: {
        ...
        command: 'bash -c "set -e; cd .. && rm -rf /tmp/zp-pw && mkdir -p /tmp/zp-pw && export ZEROPOINT_ROOT_PATH=/tmp/zp-pw ZEROPOINT_MODE=mock ZEROPOINT_XDS_PORT=18002 ZEROPOINT_AGENT_LOCAL=1; zeropoint-agent node ensure namespace settings -c name=settings --perms rw* >/dev/null; zeropoint-agent node ensure namespace modules -c name=modules --perms rw* >/dev/null; zeropoint-agent node ensure namespace system -c name=system --perms rw- >/dev/null; zeropoint-agent node ensure docker system/docker -p system --perms r-- >/dev/null; zeropoint-agent node ensure docker_network system/zeropoint_network -p system -p system/docker -c name=zeropoint-network -c driver=bridge --perms r-- >/dev/null; zeropoint-agent node ensure envoy system/envoy -p system -p system/docker -p system/zeropoint_network -c xds_port=18002 -c http_port=80 -c https_port=443 --perms r-- >/dev/null; unset ZEROPOINT_AGENT_LOCAL; zeropoint-agent serve"',
